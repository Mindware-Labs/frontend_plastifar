...
3. **Rows, not a sidebar.** The reference never runs a tall sidebar next to a tall main column — it alternates a narrow analytical module and a wider richer one, row by row. This page now does the same: KPI strip, then [Volumen de tickets (narrow) | Rendimiento operativo (wide, two panels — hourly heatmap and the SLA gauge + priority bars sharing one card, hairline-divided, not two separate cards)], then [`SystemStatusCard` (narrow) | the tickets table (wide)], each row full width before the next begins. The table finally gets a full row to itself instead of racing a column of unrelated charts for the same vertical space — that "disconnected sidebar" was the complaint this row structure answers.
