...
3. **Rows, not a sidebar.** The reference never runs a tall sidebar next to a tall main column — it alternates a narrow analytical module and a wider richer one, row by row. This page now does the same: KPI strip, then the three `mono-charts` cards (below), then the activity heatmap (also `mono-charts`, full width on its own), then [`SystemStatusCard` + a small SLA card, stacked narrow | the tickets table, wide], each row full width before the next begins. The table finally gets a full row to itself instead of racing a column of unrelated charts for the same vertical space — that "disconnected sidebar" was the complaint this row structure answers.
...
**`PriorityBars`.** A real bullet chart, not just a progress bar: each row keeps its compliance fill and semaphore tone, and now carries a fixed 2 px `ink`/40 % tick at the 90 % SLA target — the same "actual vs. threshold" idea as `MonoRoundedBulletChart` (github.com/Subhan-code/Amicro), adapted to the panel's own semaphore instead of a new accent colour. `SlaDonut` was left as it already was: a single-magnitude gauge in the same semaphore, which is what the ported `MonoRoundedGaugeArc` would have duplicated — porting it as a second, separately-carded gauge next to the SLA card was judged redundant rather than "more alive," so it wasn't added.
...
**`MonoActivityHeatmap`**, same folder, same origin, its own full-width row below the three charts above. Adapted from the source repo's activity-calendar component: weeks in columns, days in rows, a live tooltip line under the grid. Two changes beyond recolouring: the original's per-cell entrance animation runs on `motion/react`, a package this project doesn't carry, so it was dropped in favour of a plain CSS hover scale; and the colour ramp follows the rule this file already committed to above — `color-mix(in srgb, brand-red X%, white)` at 6/34/65/96 % for levels 1–4, `fill` for zero — never the original's opacity-on-black/white steps.
