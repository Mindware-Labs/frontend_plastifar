...
export function StatCard({ icon: Icon, label, value, help, tone = "brand", delta }: StatCardProps) {
  return (
    <DashboardCard padding="sm" className="flex h-full flex-col gap-3">
      <div className="flex items-start justify-between gap-2">
        <span
          className={`flex h-9 w-9 shrink-0 items-center justify-center ${INSET_RADIUS} ${chipTone[tone]}`}
        >
        ...
        <span
          ...
          className="cursor-help text-faint transition-colors hover:text-brand-gray"
        >
 ...
 *
 * Es la unica superficie tintada del panel y existe por una razon concreta: son
 * cifras de CONTEXTO —el tamano de la cartera— que enmarcan todo lo que viene
 * debajo, no metricas que pidan accion. Si fueran cinco tarjetas mas, competirian
 * en rango con las de Calidad, que si piden que alguien haga algo hoy.
 *
...
export function QuickOverview({ title, stats }: { title: string; stats: QuickStat[] }) {
  return (
    <section
      className={`${CARD_RADIUS} border border-brand-red/12 bg-gradient-to-br
        from-brand-red/[0.055] via-brand-red/[0.025] to-transparent p-5`}
    >
      <h2 className="font-heading text-[10px] font-semibold uppercase tracking-[0.08em] text-brand-red-dark">
        {title}
      </h2>
        ...
        {stats.map((stat, index) => (
          <div
            ...
            className={`px-4 first:pl-0 ${
              ...
              index % 2 === 0 ? "border-l-0" : "border-l border-brand-red/15"
